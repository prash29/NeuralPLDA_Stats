...
class NeuralPldaFromStats(nn.Module):
    def __init__(self, sdim=3000, xdim=512, LDA_dim=170, PLDA_dim=170, device=torch.device("cuda")):
        ...
        self.tdnn_11 = nn.Linear(sdim, xdim)
        self.threshold1 = nn.Parameter(torch.rand(1), requires_grad=True)  # .to(device)
        self.threshold2 = nn.Parameter(torch.rand(1), requires_grad=True)  # .to(device)
        self.threshold_Xent = nn.Parameter(torch.rand(1), requires_grad=True)  # .to(device)
    ...
    def LoadPldaParamsFromKaldi(self, mean_vec_file, transform_mat_file, PldaFile, FinalXvecPkl):
        ...
        #Without Combining
        mdsd['tdnn_11.weight'].data.copy_(torch.from_numpy(w1).float())
        mdsd['tdnn_11.bias'].data.copy_(torch.from_numpy(b1).float())

        mdsd['centering_and_LDA.weight'].data.copy_(torch.from_numpy(w2).float())
        mdsd['centering_and_LDA.bias'].data.copy_(torch.from_numpy(b2).float())

        #For combining linear layers
...
def train(args, model, device, train_loader, mega_xvec_dict, num_to_id_dict, optimizer, epoch, ):
    ...
    for batch_idx, (data1, data2, target) in enumerate(train_loader):
        ...
        loss = 0.5*(loss1+loss2) + loss_bce  # loss_bce # Change to  0.1*loss_bce #or # 0.5*(loss1+loss2) #When required
...
def main_kaldiplda():
    ...
    xvector_scp_list = np.asarray(
        ['/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_swbd/xvector_fullpaths.scp',
         '/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_sre/xvector_fullpaths.scp',
         '/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_mx6/xvector_fullpaths.scp'])
    ...
    enroll_xvector_path = "/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_sre18_dev_enrollment/xvectors.pkl"
    test_xvector_path = "/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_sre18_dev_test/xvectors.pkl"
    ...
    #    enroll_xvector_path="/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_sre16_eval_enrollment/xvectors.pkl"
    #    test_xvector_path="/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/stats_sre16_eval_test/xvectors.pkl"
    #    enroll_spk2xvector = get_spk2xvector(enroll_spk2utt_path,enroll_xvector_path)
    #    test_xvectors = pickle.load(open(test_xvector_path,'rb'))
    #    mega_xvec_dict.update(enroll_spk2xvector)
    #    mega_xvec_dict.update(test_xvectors)
    ...
    train_loader = concatenate_datasets(datasets, batch_size=8192)
    ...
    model.LoadPldaParamsFromKaldi(
        '/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/xvectors_swbd_sre_mx6/mean.vec',
        '/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/xvectors_swbd_sre_mx6/transform.mat',
        '/home/data2/SRE2019/prashantk/voxceleb/v2/exp/xvector_nnet_1a/xvectors_swbd_sre_mx6/plda',
        'Kaldi_Models/final.pkl')
    ...
    # model_state['threshold1'].data.copy_(torch.tensor(minC_threshold1).float())
    # model_state['threshold2'].data.copy_(torch.tensor(minC_threshold2).float())
    # model_state['threshold_Xent'].data.copy_(torch.tensor(min_cent_threshold).float())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        all_losses.append(valloss)
        model.SaveModel(
            "models/kaldi_NeuralPLDA_Stats_sre0410_swbd_16_{}.swbdsremx6epoch.{}.pt".format(epoch, timestamp))
        print("Generating scores for Epoch ", epoch)
        generate_scores_in_batches("scores/scores_NeuralPLDA_Stats_CUDA_Random{}_{}.txt".format(epoch, timestamp),
                                   device, sre18_dev_trials_file_path, sre18_dev_xv_pairs_1, sre18_dev_xv_pairs_2,
                                   model)
        try:
